...
function Drawer({ detail, log, tasks, profiles, boardName, comment, setComment, linkId, setLinkId, saving, onClose, mutate }) {
  ...
  return <><div className="drawer-shade" onClick={onClose}/><aside className="drawer"><header className="drawer-head"><div><span className={`status-chip status-${task.status}`}>{statusLabel(task.status)}</span><code>{task.id}</code></div><button onClick={onClose}><X size={20}/></button></header><div className="drawer-scroll">
    <form className="edit-form" onSubmit={saveFields}><input className="title-input" name="title" defaultValue={task.title}/><textarea className="body-input" name="body" rows="5" defaultValue={task.body || ''} placeholder="添加任务说明…"/><div className="property-grid"><label><span><UserRound size={14}/>负责人</span><select name="assignee" defaultValue={task.assignee || ''}><option value="">未分派</option>{profiles.map((p) => <option key={p.name}>{p.name}</option>)}</select></label><label><span><Settings2 size={14}/>优先级</span><select name="priority" defaultValue={task.priority || 0}><option value="0">普通</option><option value="1">较高</option><option value="2">高</option><option value="3">紧急</option></select></label><label><span><Clock3 size={14}/>创建时间</span><b>{displayTime(task.created_at)}</b></label><label><span><Bot size={14}/>工作区</span><b title={task.workspace_path}>{task.workspace_kind || 'scratch'}</b></label><label style={{gridColumn: '1 / -1'}}><span><Sparkles size={14}/>加载技能 (Skills)</span><b>{task.skills && task.skills.length ? task.skills.map((s) => <span key={s} style={{display:'inline-block', background:'#eef2ff', color:'#4338ca', border:'1px solid #c7d2fe', borderRadius:'4px', padding:'1px 6px', marginRight:'6px', fontSize:'12px'}}>{s}</span>) : <span style={{color:'#9ca3af', fontWeight:'normal'}}>无预加载技能 (使用默认 kanban-worker)</span>}</b></label></div><button className="secondary-button save-button" disabled={saving}>保存修改</button></form>
